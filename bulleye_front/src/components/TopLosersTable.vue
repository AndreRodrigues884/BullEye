<template>
  <div
    class="p-6 bg-gradient-to-br from-[#020826] to-[#090F30] rounded-3xl shadow-2xl w-full flex flex-col gap-4"
  >
    <!-- Cabeçalho -->
    <div class="flex items-center gap-3">
      <h3 class="text-gray-400 uppercase text-sm tracking-wider">Top Losers</h3>
      <span class="ml-auto text-gray-400 text-sm">
        {{ topLosers?.length ?? 0 }} Stocks
      </span>
    </div>

    <!-- Scroll horizontal -->
    <div class="overflow-x-auto">
      <table class="w-full min-w-[600px] border-collapse text-gray-200">
        <thead>
        ...
        <tbody>
          ...
          >
            <td class="px-4 py-2 font-semibold flex items-center gap-2 text-white">
              <img
                v-if="stock.logo"
                :src="stock.logo"
                alt="Logo"
